...
import com.lishengzn.lsdc.communication.service.APIStatusMessageReceiveService;
...
public class APIStatusReceiveServiceImpl extends AbstractCommunicateAdapter implements APIStatusMessageReceiveService {
    private static final Logger LOG = LoggerFactory.getLogger(APIStatusReceiveServiceImpl.class);
    private InputStream in ;
    private WebApplicationContext wac = ContextLoader.getCurrentWebApplicationContext();
    private VehiclePoolService vehiclePoolService= wac.getBean(VehiclePoolService.class);
    @Override
...
//        vehiclePoolService.getVehicle(this.vehicle.getVehicleIp());
    ...
    private void handleVehicleRunInfo(PacketModel packetModel) {
        ...
        vehicle.setMileage(vehicleRunInfo.getOdo());
    }
    ...
    private void handleVehicleLocation(PacketModel packetModel) {
        ...
        Coordinate coor = new Coordinate(position.getX(),position.getY());
        vehicle.setPosition(coor);
        vehicle.setConfidenceDegree(position.getConfidence());
        vehicle.setAngle(position.getAngle());
        vehicle.setCurrentStation(position.getCurrent_station());
    }
    ...
    private void handleVehicleSpeed(PacketModel packetModel) {
        ...
        vehicle.setVx(speedInfo.getVx());
        vehicle.setVy(speedInfo.getVy());
        vehicle.setAngularVelocity(speedInfo.getW());
    }
    ...
    private void handleVehicleBlock(PacketModel packetModel) {
        ...
        RequestSyncObj syncObj =vehicle.getRequestSnynObj().getQueryBlockStatusSyncObj();
        synchronized (syncObj){
            syncObj.setCarrayedMessage(packetModel.getData());
            syncObj.notifyAll();
        }
    }

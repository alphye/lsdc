...
public class ReadMapUtil {
	...
	public static String configPath = ReadMapUtil.class.getClassLoader().getResource("config").getPath();
	...
	static {
		...
		try {
			...
			nodesList.forEach((e) -> generateNodesMap(e));

			Element edges = root.element("edges");
			List<Element> edgesList = edges.elements();
			edgesList.forEach((e) -> generateEdgesMap(e));

...
public class Server_navi {
	...
	public static Server_navi getInstance() {
		return ServerInstance.serverInstance;
	}
	private ExecutorService threadPool = Executors.newCachedThreadPool();
	private static class ServerInstance {
		final static Server_navi serverInstance = getServer();

		private static Server_navi getServer() {
			try {
				return new Server_navi(19206);
			} catch (IOException e) {
				e.printStackTrace();
				return null;
			}
	 ...
	 * @throws IOException
	 */
	private Server_navi(int port) throws IOException {
	...
	public static void main(String[] args) throws Exception {
		Server_navi socketServer = Server_navi.getInstance();
		socketServer.doSocket();
	}
}

class ClientHandler_navi implements Runnable {
	...
	public ClientHandler_navi(Socket socket) {
		super();
		terminate = false;
		naviId=0L;
		this.socket = socket;

    var subscripeTopic = function() {
		...
		window.top.stomp.subscribe(constantsJSON.rtopicAddressConstants.setVehicleInfo,function(msg){
            ...
            $("#vehicle_position").html(message.position.x + ","+ message.position.y);
            // 更新小车位置
            SVGFrame.updateVehiclePosition("vehicle-01",{'X':message.position.x,'Y':message.position.y});
            $("#vehicle_vx").html(message.vx);
            $("#vehicle_vy").html(message.vy);
            $("#vehicle_batteryResidues").html(message.batteryResidues);
	...
	function move(op){
		...
		var isBackForward =$("#isBackForward").is(':checked');
		var command={"commandType":"vehicleMove","message":{"vehicleIp":$("#vehicleIp").val(),"direct":op,"cellNum":$("#moveCells").val(),"backForward":isBackForward}};
	...
	function moveToPoint(coor){
		...
		var command={"commandType":"sendToNode","message":{"vehicleIp":$("#vehicleIp").val(),"destCoor":{"x":coor.X,"y":coor.Y}}};
	...
	function appendTrail(op){
		...
		var command={"commandType":"appendNaviTaskWidthCellNum","message":{"vehicleIp":$("#vehicleIp").val(),"direct":op,"cellNum":$("#appendCells").val()}};
		send(constantsJSON.stopicAddressConstants.appendNaviTaskWidthCellNum,JSON.stringify(command));
	}
	
	function modifyNaviTask(op){
		var command={"commandType":op,"message":{"vehicleIp":$("#vehicleIp").val()}};
		var stopic="";
	...
	function beltRotationOn(){
		var command={"commandType":"sendOperationTask","message":{"vehicleIp":$("#vehicleIp").val(),"operationCode":0x021,"operationParams":[1,2]}};
		send(constantsJSON.stopicAddressConstants.sendOperationTask,JSON.stringify(command));
	}
	function beltRotationStop(){
		var command={"commandType":"sendOperationTask","message":{"vehicleIp":$("#vehicleIp").val(),"operationCode":0x022,"operationParams":[1]}};
		send(constantsJSON.stopicAddressConstants.sendOperationTask,JSON.stringify(command));
	}
	function beltRotationOnAndStop(){
		var command={"commandType":"sendOperationTask","message":{"vehicleIp":$("#vehicleIp").val(),"operationCode":0x023,"operationParams":[1,2,3000]}};
		send(constantsJSON.stopicAddressConstants.sendOperationTask,JSON.stringify(command));
	}
	...
	function clearError(){
		var command={"commandType":"clearError","message":{"vehicleIp":$("#vehicleIp").val()}};
		send(constantsJSON.stopicAddressConstants.clearError,JSON.stringify(command));
	}
	
	function queryNaviTrails(){
		var command={"commandType":"queryNaviTrails","message":{"vehicleIp":$("#vehicleIp").val()}};
	...
	function changeContainerDirection(direction){
		var command={"commandType":"sendOperationTask","message":{"vehicleIp":$("#vehicleIp").val(),"operationCode":0x031,"operationParams":[direction]}};
		send(constantsJSON.stopicAddressConstants.sendOperationTask,JSON.stringify(command));
	}
	function stopCharge(){
		var command={"commandType":"stopCharge","message":{"vehicleIp":$("#vehicleIp").val()}};
		send(constantsJSON.stopicAddressConstants.stopCharge,JSON.stringify(command));
	}
	function startCharge(){
		var command={"commandType":"startCharge","message":{"vehicleIp":$("#vehicleIp").val()}};
	...
	function writePackageSize(packageSize){
		...
		var command={"commandType":"packageSize","message":{"vehicleIp":$("#vehicleIp").val(),"packageWidth":packageSize.packageWidth,"packageLength":packageSize.packageLength}};
